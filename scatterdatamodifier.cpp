...
ScatterDataModifier::ScatterDataModifier(Q3DScatter *scatter, vector<Triangle>* meshVector, VectorC3 sol)
    : m_graph(scatter),
      m_fontSize(40.0f),
...
void ScatterDataModifier::addData()
{
    ...
    float min_tmp =m_sol.X[0].min().real();
    float max_tmp =m_sol.X[0].max().real();
    ...
    for (int i=0; i !=m_meshVector->size();i++)
    {
        ...
        Triangle tmp_tri = m_meshVector->at(i);
        ptrToDataArray->setPosition(tmp_tri.vertices[j].p);
        ptrToDataArray++;

        float x_tmp =m_sol.X[0][i].real();

        m_graph->seriesList().at(3*i+j)->setBaseColor(QColor(255,int(((x_tmp-min_tmp)/range)*255),int(((x_tmp-min_tmp)/range)*255)));
        m_graph->seriesList().at(3*i+j)->dataProxy()->resetArray(dataArray);
