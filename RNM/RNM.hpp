...
namespace RNM {
template<class T> inline T RNM_Min (const T &a,const T &b){return a < b ? a : b;}
template<class T> inline T RNM_Max (const T &a,const T & b){return a > b ? a : b;}
template<class T> inline T RNM_Abs (const T &a){return a <0 ? -a : a;}

template<class T> inline void Exchange (T& a,T& b) {T c=a;a=b;b=c;}
template<class T> inline T RNM_Max (const T &a,const T & b,const T & c){return RNM_Max(RNM_Max(a,b),c);}
template<class T> inline T RNM_Min (const T &a,const T & b,const T & c){return RNM_Min(RNM_Min(a,b),c);}
// specialisation cas complex ---
template<class T> 
inline complex<T> RNM_Min(const complex<T> &a,complex<T> &b)
{ return complex<T>(min(a.real(),b.real()),min(a.imag(),b.imag()));}
template<class T> 
inline complex<T> RNM_Max(const complex<T> &a,const complex<T> &b)
{ return complex<T>(max(a.real(),b.real()),max(a.imag(),b.imag()));}

/*inline complex<double> RNM_Min(const complex<double> &a,complex<double> &b)
{ return complex<double>(RNM_Min(real(a),real(b)),RNM_Min(imag(a),imag(b)));}
inline complex<double> RNM_Max(const complex<double> &a,const complex<double> &b)
{ return complex<double>(RNM_Max(real(a),real(b)),RNM_Max(imag(a),imag(b)));}
*/ }
//  ----                                                                                                                                             
...
template<class DATA> struct AddTo {DATA & d; }; // class to add and set Array
//  DATA deux membre .addto(KN_<R> x),  .size()  
...
public:
  ...
  template<class D> KN_& operator  =(const AddTo<D> & u) { operator=(R()) ;   return  u.addto(this); }
  template<class D> KN_& operator  +=(const AddTo<D> & u) {  return  u.addto(this); }
...
class KN :public KN_<R> { public:
 ...
 // explicit  KN(const R & u):KN_<R>(new R(uu),1,0) {}
  ...
  template<class D> KN& operator  =(const AddTo<D> & u) {  if(this->unset()) this->set(new R[u.size()],u.size());operator=(R()) ;   return  u.addto(this); }
  template<class D> KN& operator  +=(const AddTo<D> & u) { if(this->unset()) this->set(new R[u.size()],u.size()); return  u.addto(this); }
...
//Adding stuff to RNM for C3 compliance
/*#include "C3.h"

C3 inline Mulc_KN_<C3> operator/(const KN_<C3> &a, const C3 &b)
{return Mulc_KN_<C3>(a,((b*)/(b,b)));}
*/
